  ...
  defmodule Cldr.Plug.SetLocale do
    ...
    def init(options) do
      ...
      if :gettext in options[:apps] and is_nil(options[:gettext]) do
        raise ArgumentError, "The option :gettext that specified a Gettext module must be set " <>
    ...
    defp fetch_param(%Plug.Conn{query_params: %Plug.Conn.Unfetched{aspect: :query_params}} = conn,
        :query, param, options) do
    ...
    defp validate_session_key(options, nil), do: Keyword.put(options, :session_key, @default_session_key)
