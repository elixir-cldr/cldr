    ...
    case one_rule do
      ...
      {:.., _context, [from, to]} ->
        test "#{inspect module}: Validate range #{inspect from}..#{inspect to} is in plural category #{inspect category} for locale #{inspect locale_name}" do
          assert unquote(module).plural_rule(unquote(Macro.escape(from)),
            unquote(Macro.escape(locale))) == unquote(category)
          assert unquote(module).plural_rule(unquote(Macro.escape(to)),
            unquote(Macro.escape(locale))) == unquote(category)
        end
      dec ->
        test "#{inspect module}: Validate number #{inspect dec} is in plural category #{inspect category} for locale #{inspect locale_name}" do
          assert unquote(module).plural_rule(unquote(Macro.escape(dec)),
            unquote(Macro.escape(locale))) == unquote(category)
        end
    end
  end
