...
As of version 0.2.0, formatting of relative dates and date times is support in the `Cldr.Date.Relative` module.  The public API is `Cldr.Date.Relative.to_string/2`.  Some examples:
...
Tests are run on Elixir 1.4.2.  `Cldr` will not run on Elixir version before 1.4.
